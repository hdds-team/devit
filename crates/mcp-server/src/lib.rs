...
impl McpServer {
    ...
    pub async fn serve_stdio(&self) -> Result<()> {
        eprintln!("[MCP-DEBUG] Starting MCP server on STDIN/STDOUT");
        let stdin = io::stdin();
        ...
        loop {
            ...
            match reader.read_line(&mut line).await {
                Ok(0) => {
                    eprintln!("[MCP-DEBUG] Client disconnected (EOF)");
                    break;
                }
                Ok(_) => {
                    ...
                    eprintln!("[MCP-DEBUG] Received line: {}", trimmed);

                    match serde_json::from_str::<Value>(trimmed) {
                        Ok(request) => {
                            eprintln!("[MCP-DEBUG] JSON parsed successfully");
                            match self.handle_jsonrpc(request).await {
                                Ok(Some(response)) => {
                                    let response_line = serde_json::to_string(&response)? + "\n";
                                    eprintln!("[MCP-DEBUG] Sending: {}", response_line.trim());
                                    stdout.write_all(response_line.as_bytes()).await?;
                                    stdout.flush().await?;
                                    eprintln!("[MCP-DEBUG] Response sent successfully");
                                }
                                Ok(None) => {
                                    eprintln!("[MCP-DEBUG] No response emitted for this request");
                                }
                                Err(err) => {
                                    eprintln!("[MCP-ERROR] ERROR in handle_jsonrpc: {}", err);
                        ...
                        Err(err) => {
                            eprintln!("[MCP-ERROR] JSON PARSE ERROR: {}", err);
                            eprintln!("[MCP-ERROR] Raw line was: '{}'", trimmed);
                            let error_response = json!({
                                "jsonrpc": "2.0",
                ...
                Err(err) => {
                    eprintln!("[MCP-ERROR] READ ERROR: {}", err);
                    break;
                }
            }
        }

        eprintln!("[MCP-DEBUG] Client handler exiting");
        Ok(())
    }
