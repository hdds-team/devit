...
enum WorkingDirSource {
    ...
    EnvVar(&'static str),
    CoreConfig,
    AutoProjectRootFromCwd,
    AutoProjectRootFromExe,
    CurrentDir,
}
...
impl WorkingDirSource {
    fn as_str(&self) -> &'static str {
        match self {
            WorkingDirSource::CliFlag => "cli:--working-dir",
            WorkingDirSource::EnvVar(name) => name,
            WorkingDirSource::CoreConfig => "config:workspace.sandbox_root",
            WorkingDirSource::AutoProjectRootFromCwd => "auto:project_root_from_cwd",
            WorkingDirSource::AutoProjectRootFromExe => "auto:project_root_from_exe",
            WorkingDirSource::CurrentDir => "fallback:current_dir",
        }
    }
}
...
async fn main() -> Result<()> {
    ...
    let (working_dir, working_dir_source) = match &args.working_dir {
        Some(dir) => {
            ...
            (canonical, WorkingDirSource::CliFlag)
        }
        None => resolve_working_dir(args.config_path.as_ref())?,
    };

    tracing::info!(
        "Workspace root resolved: {} (source: {})",
        working_dir.display(),
        working_dir_source.as_str()
    );
    ...
    tool_options.allowed_paths = parse_allowed_paths(core_config_path.as_ref());
    if !tool_options.allowed_paths.is_empty() {
        tracing::info!("Allowed paths: {:?}", tool_options.allowed_paths);
    }
...
fn resolve_working_dir(config_path: Option<&PathBuf>) -> Result<(PathBuf, WorkingDirSource)> {
    ...
    if let Some(root) = find_project_root(&current) {
        return Ok((root, WorkingDirSource::AutoProjectRootFromCwd));
    }

    if let Ok(exe) = env::current_exe() {
        if let Some(dir) = exe.parent() {
            if let Some(root) = find_project_root(dir) {
                return Ok((root, WorkingDirSource::AutoProjectRootFromExe));
            }
        }
    }

    Ok((current, WorkingDirSource::CurrentDir))
}

fn resolve_from_env(config_path: Option<&PathBuf>) -> Result<Option<(PathBuf, WorkingDirSource)>> {
    let candidates = [
        "DEVIT_WORKDIR",
        "DEVIT_SANDBOX_ROOT",
        "DEVIT_PROJECT_ROOT",
    ...
    if let Some(config_root) = resolve_from_core_config(config_path)? {
        return Ok(Some((config_root, WorkingDirSource::CoreConfig)));
    }
...
/// Parse allowed_paths from the config file
fn parse_allowed_paths(cli_config_path: Option<&PathBuf>) -> Vec<PathBuf> {
    let config_path = if let Some(path) = cli_config_path {
        Some(path.clone())
    } else {
        env::var("DEVIT_CORE_CONFIG")
    ...
    let Some(workspace) = parsed.get("workspace").and_then(|v| v.as_table()) else {
        return vec![];
    };

    let Some(paths) = workspace.get("allowed_paths").and_then(|v| v.as_array()) else {
        return vec![];
    };
