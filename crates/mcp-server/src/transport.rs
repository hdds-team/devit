...
pub fn determine_transport(
    ...
    match transport_choice.as_str() {
        ...
        "http" => {
            ...
            enforce_http_auth_policy(&host, auth_config.as_ref())?;
...
fn enforce_http_auth_policy(host: &str, auth: Option<&HttpAuthConfig>) -> Result<()> {
    let has_tokens = auth.map(|cfg| !cfg.tokens.is_empty()).unwrap_or(false);
    if has_tokens {
        return Ok(());
    }

    // Explicit insecure override reserved for local loopback development only.
    if is_loopback_host(host) && env_flag_enabled("DEVIT_ALLOW_HTTP_NO_AUTH") {
        tracing::warn!(
            "MCP HTTP transport running without auth on loopback host '{}'; DEVIT_ALLOW_HTTP_NO_AUTH is set",
            host
        );
        return Ok(());
    }
    ...
    if !is_loopback_host(host) {
        return Err(anyhow!(
            "Refusing insecure MCP HTTP transport on non-loopback host '{}': configure bearer auth tokens (--auth-token/--tokens-file or mcp_server.auth)",
            host
        ));
    }

    Err(anyhow!(
        "MCP HTTP transport requires bearer auth tokens by default. Configure --auth-token/--tokens-file (or mcp_server.auth). For local loopback development only, set DEVIT_ALLOW_HTTP_NO_AUTH=1."
    ))
}
...
mod tests {
    ...
    fn cli_overrides_host_and_port() {
        ...
        let _allow_http_no_auth = EnvVarGuard::set("DEVIT_ALLOW_HTTP_NO_AUTH", None);
        ...
        cli.tokens = vec!["local-token".to_string()];
    ...
    #[test]
    fn http_requires_auth_by_default_even_on_loopback() {
        let _lock = ENV_LOCK.lock().expect("env lock");
        let _allow_http_no_auth = EnvVarGuard::set("DEVIT_ALLOW_HTTP_NO_AUTH", None);
        ...
        let err = determine_transport(&cli, None, Path::new("."))
            .expect_err("http without auth should be rejected");
        assert!(
            err.to_string()
                .contains("requires bearer auth tokens by default"),
            "unexpected error: {err:#}"
        );
    }

    #[test]
    fn http_without_auth_can_be_enabled_only_on_loopback_with_override() {
        let _lock = ENV_LOCK.lock().expect("env lock");
        let _allow_http_no_auth = EnvVarGuard::set("DEVIT_ALLOW_HTTP_NO_AUTH", Some("1"));

        let mut cli = CliTransportOptions::default();
        cli.transport = Some("http".to_string());
        cli.host = Some("localhost".to_string());

        let transport = determine_transport(&cli, None, Path::new("."))
            .expect("loopback override should allow insecure http");
        match transport {
            Transport::Http(http) => {
    ...
    #[test]
    fn http_without_auth_refused_on_non_loopback_even_with_override() {
        let _lock = ENV_LOCK.lock().expect("env lock");
        let _allow_http_no_auth = EnvVarGuard::set("DEVIT_ALLOW_HTTP_NO_AUTH", Some("1"));
        ...
        let err = determine_transport(&cli, None, Path::new("."))
            .expect_err("non-loopback host without auth should be rejected");
        assert!(
            err.to_string()
                .contains("Refusing insecure MCP HTTP transport on non-loopback"),
            "unexpected error: {err:#}"
        );
    }
}
