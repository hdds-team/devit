...
fn json_ok<T: Serialize>(data: T) -> Result<serde_json::Value> {
    Ok(serde_json::to_value(StdResponse {
        success: true,
        timestamp: Utc::now(),
        request_id: Some(Uuid::new_v4()),
        data: Some(data),
...
fn json_err(e: devit_cli::core::errors::DevItError) -> Result<serde_json::Value> {
    let std_error = std_error_from_devit_error(e);
    Ok(serde_json::to_value(StdResponse::<()> {
        success: false,
        timestamp: Utc::now(),
...
async fn handle_snapshot(core: &CoreEngine) -> Result<serde_json::Value> {
    match core.snapshot_get(None).await {
        Ok(snapshot_id) => json_ok(SnapshotResponse {
...
async fn handle_snapshot_extended(
    ...
    match action {
        ...
        Some(SnapshotCommands::List) => match core.snapshot_list().await {
            Ok(snapshots) => json_ok(SnapshotListResponse {
                snapshots: snapshots
                    .iter()
                    .map(|s| SnapshotEntry {
                        id: s.id.0.clone(),
                        ...
                        created_at: {
                            let dt: chrono::DateTime<chrono::Utc> = s.created_at.into();
                            dt.to_rfc3339()
                        },
                    })
                    .collect(),
        ...
        Some(SnapshotCommands::Config { max }) => {
            match core.snapshot_config(max.map(|m| m as usize)).await {
                Ok(current_max) => json_ok(SnapshotConfigResponse {
                    max_snapshots: current_max,
                    updated: max.is_some(),
                }),
                Err(e) => json_err(e),
            }
