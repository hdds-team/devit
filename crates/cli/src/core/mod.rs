...
impl CoreEngine {
    ...
    pub async fn snapshot_list(&self) -> DevItResult<Vec<snapshot::SnapshotInfo>> {
        let manager = self.snapshot_manager.read().await;
        manager.list_snapshots()
    }

    /// Returns the current max-snapshots limit, optionally updating it.
    pub async fn snapshot_config(&self, new_max: Option<usize>) -> DevItResult<usize> {
        if let Some(max) = new_max {
            let mut manager = self.snapshot_manager.write().await;
            *manager = snapshot::SnapshotManager::new(manager.snapshot_dir().to_path_buf(), max);
            Ok(max)
        } else {
            let manager = self.snapshot_manager.read().await;
            Ok(manager.max_snapshots())
        }
    }
