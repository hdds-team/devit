...
impl Default for ExecToolConfig {
    fn default() -> Self {
        Self {
            rlimit_nofile: 8192,  // Increased for cargo/rustc (many open files during builds)
            rlimit_nproc: 0,      // 0 = inherit from parent (don't restrict nproc)
            rlimit_as_gb: 16,     // 16GB for large compilations
            rlimit_cpu_secs: 900, // 15 minutes for builds
