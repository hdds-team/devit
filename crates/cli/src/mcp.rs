...
impl McpClient {
    pub fn spawn_cmd(cmd: &str, per_msg_timeout: Duration) -> Result<Self> {
        // Platform shell wrapper: bash -c on Unix, bash or cmd /C on Windows.
        // On Windows, prefers Git Bash for bash-specific syntax compatibility.
        let mut child = shell::shell_command(cmd)
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
