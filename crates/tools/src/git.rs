...
/// Representation of a `git apply --numstat` line
#[derive(Debug, Clone)]
...
/// Verify that the patch applies cleanly (dry-run)
pub fn apply_check(patch: &str) -> Result<bool> {
    let (ok, out) = run_git_with_patch(&["apply", "--check", "-"], patch)?;
    if !ok {
        return Err(anyhow!("git apply --check failed:\n{out}"));
    }
    Ok(true)
}

/// Returns the details of files affected by the patch
...
/// Apply and stage the patch. Attempts --3way fallback if --index fails.
pub fn apply_index(patch: &str) -> Result<bool> {
    let (ok, out) = run_git_with_patch(&["apply", "--index", "-"], patch)?;
    if ok {
        return Ok(true);
    }
    // Fallback 3-way (useful if index/worktree don't match perfectly)
...
/// Apply the patch to the worktree (without staging)
pub fn apply_worktree(patch: &str) -> Result<bool> {
    let (ok, out) = run_git_with_patch(&["apply", "-"], patch)?;
    if ok {
        return Ok(true);
    }
    // Fallback 3-way (useful in presence of minor divergences)
