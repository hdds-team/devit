...
#[cfg(windows)]
pub fn read_proc_stat(pid: u32) -> io::Result<ProcStat> {
    // SAFETY: OpenProcess/GetProcessTimes/CloseHandle are Win32 API calls.
    // - OpenProcess: returns null on failure (falls back to LIMITED_INFORMATION).
    // - FILETIME structs: zeroed() is valid (POD types, all-zeros = epoch).
    // - GetProcessTimes: requires valid handle (ensured above), writes to properly
    //   aligned FILETIME pointers. Returns 0 on failure (checked).
    // - CloseHandle: called on the valid handle, returns 0 on failure (checked).
    unsafe {
...
#[cfg(windows)]
pub fn process_exists(pid: u32) -> bool {
    // SAFETY: OpenProcess with PROCESS_QUERY_LIMITED_INFORMATION returns null on failure
    // (checked below) or a valid HANDLE. CloseHandle is called on the valid handle to
    // release the resource. No memory is accessed through the handle.
    unsafe {
...
fn enforce_no_symlinks(root: &Path, target: &Path) -> io::Result<()> {
    ...
    // System symlinks that are safe to allow (standard FHS directories)
    let allowed_system_symlinks = [
        "/bin",
        "/sbin",
        "/lib",
        "/lib64",
    ...
    fn is_allowed_system_symlink(path: &Path, allowed: &[&str]) -> bool {
        let path_str = path.to_string_lossy();
        allowed.iter().any(|&s| path_str == s)
    }
        ...
        match std::fs::symlink_metadata(&current) {
            Ok(metadata) => {
                if metadata.file_type().is_symlink() {
                    // Allow system symlinks only if target is outside sandbox (system paths)
                    if !current.starts_with(root)
                        || is_allowed_system_symlink(&current, &allowed_system_symlinks)
                    {
                        continue;
                    }
                    return Err(io::Error::new(
