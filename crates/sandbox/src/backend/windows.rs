...
impl WindowsSandbox {
    pub fn new() -> io::Result<Self> {
        ...
        // Returns null on failure (checked below), valid HANDLE on success.
        let raw_job = unsafe { CreateJobObjectW(std::ptr::null(), std::ptr::null()) };
        if raw_job.is_null() {
            return Err(io::Error::last_os_error());
        }
        // SAFETY: raw_job is a valid non-null HANDLE from CreateJobObjectW (checked above).
        // OwnedHandle takes ownership and will close it on drop.
    ...
    fn apply_cpu_limit(&self, percent: u32) -> io::Result<()> {
        ...
        let rate = percent.saturating_mul(100);
        // SAFETY: zeroed() is valid for JOBOBJECT_CPU_RATE_CONTROL_INFORMATION (POD struct,
        // all-zeros is a valid state with no flags set).
        ...
        // SAFETY: job_handle is valid (from OwnedHandle). info is a properly initialized
        // CPU_RATE_CONTROL struct with correct flags. size_of matches the struct type.
        // Returns 0 on failure (checked below).
        let result = unsafe {
    ...
    fn apply_memory_limit(&self, bytes: u64) -> io::Result<()> {
        // SAFETY: zeroed() is valid for JOBOBJECT_EXTENDED_LIMIT_INFORMATION (POD struct).
        ...
        // SAFETY: job_handle is valid (from OwnedHandle). info is properly initialized with
        // PROCESS_MEMORY limit flag. size_of matches the struct. Returns 0 on failure (checked).
        let result = unsafe {
            SetInformationJobObject(
                job_handle,
...
impl SandboxBackend for WindowsSandbox {
    ...
    fn spawn(&mut self, mut cmd: Command) -> Result<Self::Child> {
        ...
        let job_handle = self.job.as_raw_handle() as HANDLE;
        // SAFETY: Both handles are valid -- job_handle from OwnedHandle, process_handle from
        // the just-spawned Child. AssignProcessToJobObject returns 0 on failure (checked below).
...
impl ProcessHandle for WindowsChild {
    ...
    fn kill(&mut self) -> io::Result<()> {
        let job_handle = self.job.as_raw_handle() as HANDLE;
        // SAFETY: job_handle is valid (from OwnedHandle). TerminateJobObject terminates all
        // processes in the job with exit code 1. Returns 0 on failure (falls back to child.kill).
...
impl Drop for WindowsChild {
    fn drop(&mut self) {
        if self.kill_on_drop && self.child.try_wait().ok().flatten().is_none() {
            let job_handle = self.job.as_raw_handle() as HANDLE;
            // SAFETY: job_handle is valid (from OwnedHandle, not yet dropped). Best-effort
            // cleanup in Drop -- failure is silently ignored since we cannot propagate errors.
            unsafe {
