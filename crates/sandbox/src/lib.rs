...
/// Platform-specific sandbox implementations (landlock, seatbelt, AppContainer).
pub mod backend;
pub use backend::{ProcessHandle, SandboxBackend};
...
/// Execute a shell command after enforcing policy and sandbox restrictions. Returns exit code.
...
/// Like `run_shell_sandboxed` but captures stdout+stderr and returns (exit_code, output).
