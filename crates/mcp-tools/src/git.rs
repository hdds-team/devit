...
fn log_limit(params: &Value) -> McpResult<u64> {
    ...
    if value <= 0 {
        return Err(validation_error("'max_count' must be greater than 0"));
    }
...
impl McpTool for GitBlameTool {
    ...
    fn description(&self) -> &str {
        "Show git blame for a file (optionally a line range)"
    }
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        if let Some(line_start) = params.get("line_start").and_then(Value::as_i64) {
            let line_start = if line_start < 1 {
                return Err(validation_error("'line_start' must be >= 1"));
            } else {
                line_start as u64
            };
            ...
            let line_end = params
                ...
                .map(|end| {
                    if end < 1 {
                        Err(validation_error("'line_end' must be >= 1"))
                    } else if (end as u64) < line_start {
                        Err(validation_error("'line_end' must be >= line_start"))
                    } else {
                        Ok(end as u64)
                    }
                })
...
impl McpTool for GitShowTool {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        let commit = params
            .get("commit")
            .and_then(Value::as_str)
            .ok_or_else(|| missing_param("commit", "string"))?;
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        if let Some(range) = params.get("range").and_then(Value::as_str) {
            if range.trim().is_empty() {
                return Err(validation_error("'range' cannot be empty"));
            }
...
impl McpTool for GitSearchTool {
    ...
    fn description(&self) -> &str {
        "Search via git grep or git log -S"
    }
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        let pattern = params
            ...
            .map(|p| p.trim())
            .filter(|p| !p.is_empty())
            .ok_or_else(|| missing_param("pattern", "string"))?;
        ...
        let (mut args, treat_code_one_as_empty) = match mode.as_str() {
            ...
            other => {
                let message = format!("'type' not supported: {} (expected 'grep' or 'log')", other);
