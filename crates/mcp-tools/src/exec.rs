...
impl DevitExec {
    ...
    async fn execute_background_windows(&self, config: &ExecConfig) -> McpResult<Value> {
        ...
        let record_json = serde_json::to_value(&record).expect("Serialize-derived struct");
        let auto_kill_at_value = record.auto_kill_at;
        let auto_kill_at_text = auto_kill_at_value.as_ref().map(|ts| ts.to_rfc3339());
        let process_json = serde_json::to_value(&bg_proc).expect("Serialize-derived struct");

        let summary = if let Some(ref auto) = auto_kill_at_text {
            format!(
                "🚀 `{}` started in background — pid {} (pgid {}), auto-kill {}",
                config.binary, pid, pgid, auto
            )
        } else {
            format!(
                "🚀 `{}` started in background — pid {} (pgid {})",
    ...
    fn build_safe_env(
        ...
        // Inherit PATH from parent environment (needed for dev tools like cargo, node, etc.)
        // Fallback to minimal safe defaults if not available
        let default_path = if cfg!(target_os = "windows") {
            std::env::var("PATH").unwrap_or_else(|_| r"C:\Windows\System32;C:\Windows".to_string())
        } else {
            // Try to inherit PATH, fallback to sensible defaults
            std::env::var("PATH").unwrap_or_else(|_| "/usr/local/bin:/usr/bin:/bin".into())
        };
        env.insert("PATH".into(), default_path);

        // Set RUSTUP_HOME and CARGO_HOME if not already set (needed because HOME is sandboxed)
        // This allows cargo/rustup to find their installations
        #[cfg(target_family = "unix")]
        {
            if let Some(real_home) = std::env::var("HOME")
                .ok()
                .or_else(|| std::env::var("USER").ok().map(|u| format!("/home/{}", u)))
            {
                // Set RUSTUP_HOME if not already set
                if std::env::var("RUSTUP_HOME").is_err() {
                    let rustup_home = format!("{}/.rustup", real_home);
                    if std::path::Path::new(&rustup_home).exists() {
                        env.insert("RUSTUP_HOME".into(), rustup_home);
                    }
                }
                // Set CARGO_HOME if not already set
                if std::env::var("CARGO_HOME").is_err() {
                    let cargo_home = format!("{}/.cargo", real_home);
                    if std::path::Path::new(&cargo_home).exists() {
                        env.insert("CARGO_HOME".into(), cargo_home);
                    }
                }
            }
        }
    ...
    async fn spawn_process(&self, config: &ExecConfig) -> io::Result<std::process::Child> {
        ...
        // - setpgid(0,0): creates a new process group so we can signal the whole group.
        //   Valid because we are in the child process right after fork().
        // - apply_resource_limits: calls setrlimit/prctl which are safe POSIX syscalls
        //   when called with valid arguments (see apply_resource_limits SAFETY comments).
        // The closure captures only `limits` (Clone'd config, no shared mutable state).
        let child = unsafe {
    ...
    fn apply_resource_limits(limits: &ExecToolConfig) -> io::Result<()> {
        ...
        // SAFETY: setrlimit is a POSIX syscall. The rlimit struct is properly initialized
        // with valid cur/max values. Called in pre_exec child context (single-threaded).
        let ret = unsafe { setrlimit(RLIMIT_NOFILE, &nofile) };
        if ret != 0 {
            return Err(io::Error::last_os_error());
        }

        // NPROC (skip if 0 = inherit from parent)
        if limits.rlimit_nproc > 0 {
            let nproc = rlimit {
                rlim_cur: limits.rlimit_nproc as rlim_t,
                rlim_max: limits.rlimit_nproc as rlim_t,
            };
            // SAFETY: same as NOFILE above -- valid rlimit struct, POSIX syscall.
        ...
        // PR_SET_NO_NEW_PRIVS
        // SAFETY: prctl(PR_SET_NO_NEW_PRIVS, 1) is a Linux security hardening syscall that
        // prevents the child from gaining privileges via setuid/setgid. Always safe to call.
        ...
        // PR_SET_PDEATHSIG
        // SAFETY: prctl(PR_SET_PDEATHSIG, SIGKILL) ensures the child is killed when the
        // parent dies, preventing orphan processes. Always safe to call in child context.
    ...
    async fn execute_foreground_unix(&self, config: &ExecConfig) -> McpResult<Value> {
        ...
        match output_result {
            ...
            Err(_timeout) => {
                // SAFETY: pid is a valid process ID from the spawned child. Negating it
                // targets the entire process group (set via setpgid in pre_exec). SIGKILL
                // is always valid. This is a timeout cleanup path.
                // pid is from Child::id() (u32); on Linux PIDs fit in i32 (max PID_MAX_LIMIT
                // = 2^22), but we guard against theoretical overflow.
