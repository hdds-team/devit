...
impl McpTool for DevitKill {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        #[cfg(target_family = "unix")]
        {
            ...
            let pgid_i32 = i32::try_from(record.pgid).map_err(|_| {
                McpError::ExecutionFailed(format!(
                    "PGID {} exceeds i32::MAX, refusing to signal",
                    record.pgid
                ))
            })?;
            // SAFETY: record.pgid is validated against start_ticks to prevent PID reuse attacks.
            // pgid_i32 is bounds-checked above. Negating targets the process group.
            // sig is parsed from a validated signal name.
            let ret = unsafe { libc::kill(-pgid_i32, sig) };
            if ret != 0 {
                return Err(McpError::ExecutionFailed(format!(
                    "kill failed: {}",
        ...
        #[cfg(windows)]
        {
            ...
            // SAFETY: pid is validated against start_ticks to prevent PID reuse attacks.
            // OpenProcess returns null on failure (checked below).
            ...
            // SAFETY: handle is a valid non-null HANDLE from OpenProcess (checked above).
            // TerminateProcess sends exit code 1. CloseHandle releases the handle.
