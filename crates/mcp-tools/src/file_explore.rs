...
impl FileExplorer {
    ...
    pub(crate) async fn project_tree(&self, raw_path: &str) -> McpResult<ProjectNode> {
        ...
        let structure = self
            ...
            .project_structure(
                &relative,
                Some(u8::try_from(MAX_STRUCTURE_DEPTH).unwrap_or(u8::MAX)),
            )
            .await
            .map_err(|err| internal_error(err.to_string()))?;
...
impl McpTool for FileSearchExtTool {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let context_lines_u8 = u8::try_from(context_lines).unwrap_or(u8::MAX);
        let rendered = self
            .explorer
            .search_ext(
                pattern,
...
impl McpTool for ProjectStructureExtTool {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let max_depth = params
            .get("max_depth")
            .and_then(Value::as_u64)
            .and_then(|v| u8::try_from(v.min(MAX_STRUCTURE_DEPTH as u64)).ok());
