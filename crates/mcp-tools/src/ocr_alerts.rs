...
impl McpTool for OcrAlertsTool {
    ...
    fn description(&self) -> &str {
        "Trigger OCR alerts (regex) on an image (latest screenshot by default), with optional notification action."
    }

    async fn execute(&self, params: Value) -> McpResult<Value> {
        let rules_val = params
            .get("rules")
            .ok_or_else(|| validation_error("'rules' parameter is required"))?;
        let rules: Vec<RuleSpec> = serde_json::from_value(rules_val.clone())
            .map_err(|e| validation_error(&format!("Invalid 'rules' parameter: {}", e)))?;

        if rules.is_empty() {
            return Err(validation_error("'rules' parameter cannot be empty"));
        }
    ...
    fn input_schema(&self) -> Value {
        json!({
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": "Image path (default: latest screenshot)"},
                "lang": {"type": "string", "default": "eng"},
                "psm": {"type": "integer"},
                "inline": {"type": "boolean", "default": true},
                "task_id": {"type": "string", "description": "Optional: task_id to trigger devit_notify(status=progress)"},
                "rules": {
                    "type": "array",
                    "items": {
                        "type": "object",
