...
use crate::errors::{internal_error, io_error, missing_param, validation_error};
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "delegated_to": {
                    "type": "string",
                    "description": "Worker identifier (e.g., claude_code, codex, ...)",
                    "default": "claude_code"
                },
                "timeout": {
                    "type": "integer",
                ...
                "model": {
                    "type": "string",
                    "description": "Model name to use (optional override)"
                },
                "watch_patterns": {
                    "type": "array",
                    "items": {"type": "string"}
                ...
                "working_dir": {
                    "type": "string",
                    "description": "Working directory relative to sandbox (e.g., project-a/tests)"
                },
                "format": {
                    "type": "string",
...
fn parse_working_dir(raw: Option<&Value>, fs: &FileSystemContext) -> McpResult<Option<PathBuf>> {
    ...
    if dir.trim().is_empty() {
        return Err(validation_error(
            "The 'working_dir' parameter cannot be empty",
        ));
    }
    ...
    let metadata = std::fs::metadata(&canonical).map_err(|err| {
        io_error(
            ...
            Some(&canonical),
            format!("Cannot access directory: {err}"),
        )
    })?;

    if !metadata.is_dir() {
        return Err(validation_error(
            "The 'working_dir' parameter must reference a directory",
        ));
    }

    let relative = canonical
        .strip_prefix(fs.root())
        .map_err(|_| internal_error("Path outside sandbox"))?;
...
impl McpTool for NotifyTool {
    ...
    fn description(&self) -> &str {
        "Notify the orchestrator of delegated task progress"
    }
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        let task_id = params
            ...
            .ok_or_else(|| missing_param("task_id", "string"))?;

        let status = params
            .get("status")
            .and_then(Value::as_str)
            .ok_or_else(|| missing_param("status", "string"))?;
        ...
        let summary = params
            ...
            .and_then(Value::as_str)
            .filter(|value| !value.trim().is_empty())
            .ok_or_else(|| missing_param("summary", "string"))?;
        ...
        let details = match params.get("details") {
            ...
            Some(_) => {
                return Err(validation_error(
                    "The 'details' parameter must be a JSON object",
                ))
            }
        };
        ...
        let evidence = match params.get("evidence") {
            ...
            Some(_) => {
                return Err(validation_error(
                    "The 'evidence' parameter must be a JSON object",
                ))
            }
        };
        ...
        if route_via_context {
            let context = self
                .context
                .as_ref()
                .ok_or_else(|| internal_error("No orchestration backend available"))?;
            context
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let task = loop {
            ...
            if attempts >= MAX_ATTEMPTS {
                return Err(validation_error(&format!(
                    "Task '{}' not found in orchestration",
                    task_id
                )));
            }
