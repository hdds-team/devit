...
impl FileWriteTool {
    pub fn new(context: Arc<FileSystemContext>) -> McpResult<Self> {
        let writer = SafeFileWriter::new()
            ...
            .map_err(|err| {
                internal_error(format!("SafeFileWriter initialization failed: {err}"))
            })?;
    ...
    fn parse_mode(&self, raw_mode: Option<&str>) -> McpResult<SafeWriteMode> {
        match raw_mode.unwrap_or("overwrite") {
            ...
            other => Err(validation_error(&format!(
                "Invalid mode: '{}'. Supported modes: overwrite, append, create_new",
                other
            ))),
        }
    }
...
impl McpTool for FileWriteTool {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let target_path = self.context.resolve_write_path(path_str)?;

        if target_path.is_dir() {
            return Err(validation_error("Cannot write to a directory"));
        }
        ...
        let (buffer, byte_len) = match encoding {
            ...
            "binary" => match general_purpose::STANDARD.decode(content) {
                ...
                Err(err) => {
                    return Err(validation_error(&format!("Invalid base64 encoding: {err}")))
                }
            },
            other => {
                return Err(validation_error(&format!(
                    "Invalid encoding: '{other}'. Supported encodings: utf8, binary"
                )));
            }
        };
