...
impl McpTool for ScreenshotTool {
    ...
    fn description(&self) -> &str {
        "Capture a screenshot of the current desktop via devitd (supports multiple backends)."
    }
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let thumb_width = params
            ...
            .and_then(Value::as_u64)
            .unwrap_or(480);
        let thumb_width = u32::try_from(thumb_width).unwrap_or(480);
        let _thumb_quality = params
            .get("thumb_quality")
            .and_then(Value::as_u64)
            .unwrap_or(80);
        let _thumb_quality = u8::try_from(_thumb_quality).unwrap_or(80);
        ...
        let mut content = vec![json!({
            "type": "text",
            "text": format!("📸 Capture saved ({human_size}) — {path}", path = path_str)
        })];
    ...
    fn input_schema(&self) -> Value {
        json!({
            "type": "object",
            "properties": {
                "inline": { "type": "boolean", "description": "Include base64 thumbnail if size compatible", "default": true },
                "max_inline_kb": { "type": "integer", "description": "Max threshold (KB) for base64 inline", "default": 512 },
                "thumb_width": { "type": "integer", "description": "Max thumbnail width (px)", "default": 480 },
                "thumb_quality": { "type": "integer", "description": "JPEG thumbnail quality (1-100)", "default": 80 }
            },
