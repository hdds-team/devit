...
pub struct ToolOptions {
    ...
    /// Additional paths to allow access to (beyond sandbox_root)
...
impl WorkerBridge {
        ...
        if auto_start_allowed {
                ...
                DaemonBackend::ensure_daemon_running(&socket_str, true, timeout_ms).await
            {
                tracing::warn!(
                    %socket_str,
                    %worker_id,
                    "Unable to launch devitd automatically: {err}"
                );
                return Err(err);
            }
        }
            ...
            match self.client.poll().await {
                ...
                Err(err) => {
                    return Err(internal_error(format!(
                        "Unable to retrieve tasks from daemon: {err}"
                    )));
                }
            }
    ...
    pub async fn notify(
        ...
        self.client
            ...
            .await
            .map_err(|err| internal_error(format!("Unable to send notification: {err}")))?;
    ...
    async fn handle_delegate(&self, msg: Msg) -> McpResult<()> {
        ...
        info!("Task received for {}: {}", self.worker_id, task.task_id);
    ...
    fn build_task(&self, msg: &Msg) -> McpResult<WorkerTask> {
        let task_payload = msg
            ...
            .ok_or_else(|| internal_error("Invalid task payload received from daemon"))?;
...
impl mcp_core::McpTool for PollTasksTool {
    ...
    fn description(&self) -> &str {
        "Retrieves the next task assigned to the worker connected to the daemon"
    }
        ...
        match self.worker.poll_event(wait).await? {
            Some(WorkerEvent::Assignment(task)) => {
                ...
                Ok(json!({
                    "content": [
                        {
                            "type": "text",
                            "text": "✅ New task assigned by daemon"
                        },
                        {
                            "type": "json",
                            "json": payload
                ...
                "content": [
                    {
                        "type": "text",
                        "text": format!(
                            "⚠️ Task {} cancelled by orchestrator{}",
                            task_id,
                            reason
                                .as_ref()
                                .map(|r| format!(" : {}", r))
            ...
            Some(WorkerEvent::Timeout { task_id }) => Ok(json!({
                "content": [
                    {
                        "type": "text",
                        "text": format!("⏱️ Task {} timed out at orchestrator", task_id)
                    },
                    {
                        "type": "json",
                        "json": json!({
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "wait": {
                    "type": "boolean",
                    "description": "Wait a few seconds for a task if none is immediately available",
                    "default": false
                }
            },
